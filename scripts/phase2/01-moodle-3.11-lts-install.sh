...
# =============================================================================
# LMSK2 Moodle Server - Moodle 3.11 LTS Installation Script
# =============================================================================
# Description: Install Moodle 3.11 LTS with optimal configuration
# Author: jejakawan007
# Version: 1.0
# Date: September 2025
# =============================================================================
...
BACKUP_DIR="/backup/lmsk2/moodle-install"
MOODLE_VERSION="3.11"
MOODLE_DOWNLOAD_URL="https://download.moodle.org/releases/latest311/moodle-latest-311.tgz"
...
download_moodle() {
    log "INFO" "Downloading Moodle $MOODLE_VERSION LTS..."
    
    cd /tmp/moodle-install
    
    # Download Moodle
    if wget -O moodle-latest-311.tgz "$MOODLE_DOWNLOAD_URL"; then
    ...
    # Verify download
    if [[ -f "moodle-latest-311.tgz" ]]; then
        local file_size=$(stat -c%s "moodle-latest-311.tgz")
    ...
    if tar -xzf moodle-latest-311.tgz; then
...
// Timezone
\$CFG->timezone = '${TIMEZONE:-Asia/Jakarta}';
...
run_moodle_installation() {
    ...
    if sudo -u www-data php admin/cli/install.php \
        ...
        --dbpass="${DB_PASSWORD:-}" \
        --fullname="${SITE_FULLNAME:-LMS K2 Moodle Server}" \
        --shortname="${SITE_SHORTNAME:-LMS}" \
...
0 2 * * * /usr/bin/php $MOODLE_DIR/admin/cli/maintenance.php --enable
0 3 * * * /usr/bin/php $MOODLE_DIR/admin/cli/upgrade.php --non-interactive
0 4 * * * /usr/bin/php $MOODLE_DIR/admin/cli/maintenance.php --disable
EOF
    ) | crontab -
...
configure_initial_settings() {
    log "INFO" "Configuring initial Moodle settings..."
    
    cd "$MOODLE_DIR"
    
    # Set site settings
    sudo -u www-data php admin/cli/cfg.php --name=sitename --set="${SITE_FULLNAME:-LMS K2 Moodle Server}"
    ...
    sudo -u www-data php admin/cli/cfg.php --name=noreplyaddress --set="noreply@${MOODLE_DOMAIN:-example.com}"
    ...
    # Configure timezone
    sudo -u www-data php admin/cli/cfg.php --name=timezone --set="${TIMEZONE:-Asia/Jakarta}"
    
    # Configure language
    sudo -u www-data php admin/cli/cfg.php --name=lang --set="${LANGUAGE:-en}"
    
    # Clear caches
    sudo -u www-data php admin/cli/purge_caches.php
    
    log "INFO" "Initial Moodle settings configured successfully"
}
...
verify_installation() {
    ...
    if mysql -u "${DB_USER:-moodle}" -p"${DB_PASSWORD:-}" "${DB_NAME:-moodle}" -e "SELECT COUNT(*) as tables FROM information_schema.tables WHERE table_schema = '${DB_NAME:-moodle}';" >/dev/null 2>&1; then
...
main() {
    log "INFO" "Starting Moodle $MOODLE_VERSION LTS installation process..."
    ...
    log "INFO" "Moodle $MOODLE_VERSION LTS installation process completed successfully!"
...
case "${1:-}" in
    ...
    --dry-run)
        ...
        log "INFO" "Would execute: download_moodle, install_moodle_files, create_moodle_config, run_moodle_installation, configure_moodle_cron, run_post_installation, configure_initial_settings"
        exit 0
        ;;
    --config)
